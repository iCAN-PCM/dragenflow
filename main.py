...
# register flows
available_flows = {"echo": constructFlowEcho(), "head": constructFlowHead(), "dragen":constructDragen()}
...
class dragenflow(object):
    def parse_file(self, path: str, flow:str ) -> None:
        if flow == "dragen":
            ...
            data_file['sort_order'] = data_file['tumor/normal'].apply(lambda x: custom_sort(x))
            data_file['sort_order'] =  data_file['sort_order'].astype(float)
            data_file = data_file.sort_values(['sort_order','tumor/normal']).reset_index(drop=True)
